...
export function Fortune30Settings() {
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4">
        <h3 className="text-lg font-medium">Fortune 30 Partners</h3>
          ...
          {partners.map(partner => (
            <TooltipProvider key={partner.id}>
              <Tooltip>
                ...
                <TooltipContent>
                  <p>{partner.role || 'Strategic Partner'}</p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          ))}
      ...
      <EditPartnerDialog
        ...
        onUpdate={async (updated) => {
          try {
            await db.updateCollaborator(updated.id, updated);
            await refetch();
            setEditPartner(null);
            toast({
              title: "Success",
